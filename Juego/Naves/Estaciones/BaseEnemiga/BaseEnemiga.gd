...
func spawnear_orbital() -> void:
	numer_orbitales -= 1
	$RutaEnemigos.global_position = global_position
...
func deteccion_cuadrante() -> Vector2:
	...
	if abs(angulo_player) <= 45.0:
		# por derecha
		$RutaEnemigos.rotation_degrees = 180.0
		return $PosicionesSpawn/Este.position
	elif abs(angulo_player) > 135.0 and abs(angulo_player) <= 180:
		# por izquierda
		$RutaEnemigos.rotation_degrees = 0.0
		return $PosicionesSpawn/Oeste.position
	elif abs(angulo_player) > 45.0 and abs(angulo_player) <= 135:
		# por abajo
		if sign(angulo_player) > 0:
			$RutaEnemigos.rotation_degrees = 270.0
			return $PosicionesSpawn/Sur.position
		else:
			$RutaEnemigos.rotation_degrees = 90.0
